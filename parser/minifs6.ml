...
let rec do_all ct gamma lst = 
  match lst with
  | [] -> ()
  | x :: xs -> Printf.printf "----------------FUNC: %s ------------------" x.func_name;
...
let make_offer : func = {requires = And(Equals(TmGetParam("sender"), TmVar("InstanceBuyer")),And(Equals(TmGetState("ItemAvailable"), TmVar("CurrentState")), Greater(TmGetParam("param_offer"), TmVar("AskingPrice")))); return_type = TUnit; func_name = "make_offer"; params = [param_offer]; 
...
let reject : func = {requires = And(Equals(TmGetParam("sender"), TmVar("InstanceOwner")), And(Equals(TmGetState("OfferPlaced"), TmVar("CurrentState")),Not(Equals(param_offer_r_val, TmValue(VCons(0)))))); return_type = TUnit; func_name = "reject"; params = [param_offer_r]; body = TmSeq(TmAssign("InstanceBuyer", TmValue(VAddress("sender"))) , TmSeq( TmAssign("CurrentState", TmGetState("ItemAvailable")),TmReturn(TmValue(VUnit))))   }


let param_offer_a : params = (TNat, "param_offer_a")
let param_offer_a_val : term = TmGetParam("param_offer")    
let accept : func = { requires = And(Equals(TmGetState("OfferPlaced"), TmVar("CurrentState")),(Equals( sender, instanceOwner)));return_type = TUnit; func_name = "accept"; params = [param_offer_a]; body = TmSeq(TmIf((Equals(param_offer_a_val, TmValue(VCons(0)))), 
                                                        TmRevert, 
