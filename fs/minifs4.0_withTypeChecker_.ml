...
let rec pp_term fmt = function
  ...
  | TmReturnUnit(e) -> Format.fprintf fmt "return unit %a" pp_term e 
...
let rec typecheck whites gamma ct ty t =
  ...
  Format.eprintf "%a , %a |- %a : %a@." pp_contract ct pp_gamma gamma pp_term t pp_typ ty ; 
    begin match t with 
      ...
      | TmReturnUnit(e) -> Format.eprintf "(RETURN UNIT) @."; ignore(typecheck (whites + 2) gamma ct (Some ty) (TmReturn(e))); typecheck (whites + 2) gamma ct (Some TUnit) (TmValue(VUnit))
...
let tmapass = TmReturnUnit( TmMappAss(tmap, TmValue(VCons(13)), ttrue, TMapping(TNat,TBool)))
...
test_func gammaenv3 CT.empty (Some (TMapping(TAddress, TNat))) (mk_eval tmapass)

let tmapsel = TmMappSel(tmapp, TmVar("sender"))
test_func gammaenv3 CT.empty (Some TNat) (mk_eval tmapsel)
...
let gammaenv2 = (Gamma.add "x" (TString) Gamma.empty)
...
let bank_constructor : construtor_ = ([param_balances],TmReturnUnit(TmAssign("balances", param_balances_val, TMapping(TAddress, TNat)) )) 

let deposit : func = { return_type = TUnit; name = "deposit"; params = []; body = TmReturnUnit( TmMappAss(balances, sender, TmPlus( TmMappSel(balances, sender, TMapping(TAddress, TNat) ) , msg_value), TMapping(TAddress, TNat) ) ) }
...
let transfer : func = {return_type = TUnit; name = "transfer"; params = [param_to; param_amount]; body = TmReturnUnit( TmIf( GreaterOrEquals(TmMappSel(balances, sender,TMapping(TAddress, TNat)), param_amount_val), 
                                                                      TmSeq( TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat)),
                                                                      TmMappAss(balances, param_to_val, TmPlus( TmMappSel(balances, param_to_val,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat))), 
...
let withdraw : func = {return_type = TUnit; name = "withdraw"; params = [param_withdraw_amount]; body = TmReturnUnit( TmIf( GreaterOrEquals(TmMappSel(balances, sender,TMapping(TAddress, TNat)), param_withdraw_amount_val), 
                                                                  TmSeq(TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender,TMapping(TAddress, TNat)),param_withdraw_amount_val),TMapping(TAddress, TNat)), 
...
let constructor_body = TmReturnUnit(TmAssign("balances", param_balances_val,TMapping(TAddress, TNat)) )
test_func gammaBank ctBank (Some TUnit) (mk_eval constructor_body)

let deposit_body =  TmReturnUnit( TmMappAss(balances, sender, TmPlus( TmMappSel(balances, sender, TMapping(TAddress, TNat) ) , msg_value), TMapping(TAddress, TNat) ) ) 
...
let transfer_body = TmReturnUnit( 
  TmIf( GreaterOrEquals(TmMappSel(balances, sender,TMapping(TAddress, TNat)), param_amount_val), 
    TmSeq( TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat)),
    TmMappAss(balances, param_to_val, TmPlus( TmMappSel(balances, param_to_val,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat))), 
  TmValue(VUnit))) 
...
(*test transfer_body*)
let t11 = TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat))
let t22 = TmMappAss(balances, param_to_val, TmPlus( TmMappSel(balances, param_to_val,TMapping(TAddress, TNat)),param_amount_val),TMapping(TAddress, TNat))
let t33 = TmValue(VTrue)
let t = TmSeq(t11,t22)
test_func gammaBank ctBank (Some TUnit) (mk_eval t)


let withdraw_body = TmReturnUnit( TmIf( GreaterOrEquals(TmMappSel(balances, sender,TMapping(TAddress, TNat)), param_withdraw_amount_val), 
TmSeq(TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender,TMapping(TAddress, TNat)),param_withdraw_amount_val),TMapping(TAddress, TNat)), 
      TmTransfer( sender,param_withdraw_amount_val ) ), 
      TmReturn(TmValue(VUnit))) )
...
let marketplace_constructor : construtor_ = ([param_description;param_price], 
                                     TmReturnUnit(TmSeq( TmAssign("InstanceOwner",sender) , TmSeq(TmAssign("AskingPrice", param_price_val ), TmAssign("Description", param_description_val) )) ) )
...
(*test marketplace*)
let constructor_body =  TmReturnUnit(TmSeq( TmAssign("InstanceOwner",sender) , TmSeq(TmAssign("AskingPrice", param_price_val ), TmAssign("Description", param_description_val) )) ) 
