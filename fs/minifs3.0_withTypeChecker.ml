  ...
  | None -> for i = 0 to whites - 1 do Format.eprintf " " done ;
   Format.eprintf "%a , %a |- %a : " pp_contract ct pp_gamma gamma pp_term t; 
    begin match t with
      ...
      | Equals(e1,e2) -> Format.eprintf "(Equals) @."; 
        let te1 = typecheck (whites + 2) gamma ct None e1 in
          let te2 = typecheck (whites + 2) gamma ct None e2 in 
            if te1 == te2 then TBool else compType te1 te2
      | Greater(e1,e2) | Less(e1,e2) | LessOrEquals(e1,e2) | GreaterOrEquals(e1,e2)-> Format.eprintf "(Cond arit) @."; (*fazer equals dos termos*)
        let te1 = typecheck (whites + 2) gamma ct None e1 in
        let te2 = typecheck (whites + 2) gamma ct None e2 in 
            if te2 == TNat && te1 == TNat then TBool else compType TNat te1                     
