...
let rec typecheck whites gamma ct ty t =
  ...
  | None -> for i = 0 to whites - 1 do Format.eprintf " " done ;
   (*Format.eprintf "%a , %a |- %a : " pp_contract ct pp_gamma gamma pp_term t; (*Meter isto a aparecer uma vez só *)*)
    begin match t with
...
let rec do_all ct gamma lst = 
  match lst with
  | [] -> ()
  | x :: xs -> Printf.printf "----------------FUNC: %s ------------------" x.name; test_func gamma ct (Some TBool) (mk_eval x.requires); test_func gamma ct (Some x.return_type) (mk_eval x.body); do_all ct gamma xs

let typecheck_contract ct gamma contract = 
  Printf.printf "--------INVARIANT---------------------------\n";
  test_func gamma ct (Some TBool) (mk_eval contract.invariant);
...
let transfer : func = {requires = TmValue(VTrue); return_type = TUnit; name = "transfer"; params = [param_to; param_amount]; 
...
let contractBank : contract_tc = {
  ...
  behavioral_types = [];
  vars = [TmVar("sender"); TmVar("msg_value");TmVar("CurrentState");TmVar("balances")];
...
(*
let constructor_body =  TmSeq(TmSeq(TmAssign("CurrentState", TmGetState("ItemAvailable")) , TmSeq(TmAssign("InstanceOwner",sender), TmSeq(TmAssign("AskingPrice", param_price_val), TmAssign("Description", param_description_val)))), TmReturn(TmValue(VUnit)) ) 
test_func gammaMark ctMark (Some TUnit) (mk_eval constructor_body);;

let make_offer_requires = Equals(TmGetState("ItemAvailable"), TmVar("CurrentState"))
test_func gammaMark ctMark (Some TBool) (mk_eval make_offer_requires);;
...
let accept_offer_requires =  And(Equals(TmGetState("OfferPlaced"), TmVar("CurrentState")),Not(Equals( sender, instanceOwner)))
test_func gammaMark ctMark (Some TBool) (mk_eval accept_offer_requires);;
...
let reject_offer_requires = And(Equals(TmGetState("OfferPlaced"), TmVar("CurrentState")),Not(Equals(param_offer_r_val, TmValue(VCons(0)))))
test_func gammaMark ctMark (Some TBool) (mk_eval reject_offer_requires);;
let reject_offer_body =  TmSeq(TmAssign("InstanceBuyer", TmValue(VAddress("Sender"))) , TmReturn(TmValue(VUnit)))
...
let contractTelemetry : contract_tc = {
  ...
  vars = [TmVar("Sender"); TmVar("Owner");TmVar("CounterParty");TmVar("Device");
          TmVar("ComplianceStatus"); TmVar("Max_Temperature");TmVar("Min_Temperature")];
...
let contractMark : contract_tc = {
  ...
  vars = [TmVar("CurrentState"); TmVar("InstanceOwner");TmVar("Description");TmVar("AskingPrice");
