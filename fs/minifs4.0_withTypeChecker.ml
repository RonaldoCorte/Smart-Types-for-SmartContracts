...
let rec typecheck whites gamma ct ty t =
  ...
  | None -> for i = 0 to whites - 1 do Format.eprintf " " done ;
   Format.eprintf "%a , %a |- %a : " pp_contract ct pp_gamma gamma pp_term t; 
    begin match t with
      ...
      | TmPlus(e1,e2) -> Format.eprintf "(Plus) @.";
        let te1 = typecheck (whites + 2) gamma ct None e1 in
        Format.eprintf "%a @." pp_typ te1 ;
        let te2 = typecheck (whites + 2) gamma ct None e2 in 
        if te1 == te2 && te1 == TNat then te1 else compType TNat te1
      | TmMinus(e1,e2) -> Format.eprintf "(Minus) @.";
        let te1 = typecheck (whites + 2) gamma ct None e1 in
        Format.eprintf "%a @." pp_typ te1 ;
        let te2 = typecheck (whites + 2) gamma ct None e2 in 
        if te1 == te2 && te1 == TNat then te1 else compType TNat te1  
      | TmMult(e1,e2) -> Format.eprintf "(Mult) @.";
        let te1 = typecheck (whites + 2) gamma ct None e1 in
        Format.eprintf "%a @." pp_typ te1 ;
        let te2 = typecheck (whites + 2) gamma ct None e2 in 
        if te1 == te2 && te1 == TNat then te1 else compType TNat te1  
      | TmDiv(e1,e2) -> Format.eprintf "(Div) @.";
        let te1 = typecheck (whites + 2) gamma ct None e1 in
        Format.eprintf "%a @." pp_typ te1 ;
        let te2 = typecheck (whites + 2) gamma ct None e2 in 
        if te1 == te2 && te1 == TNat then te1 else compType TNat te1  
      ...
      | Equals(e1,e2) -> Format.eprintf "(Equals) @."; (*fazer equals dos termos*)
      let te1 = typecheck (whites + 2) gamma ct None e1 in
      Format.eprintf "%a @." pp_typ te1 ;
      let te2 = typecheck (whites + 2) gamma ct None e2 in 
      if te1 == te2 then TBool else compType te1 te2
  ...
  Format.eprintf "%a , %a |- %a : %a@." pp_contract ct pp_gamma gamma pp_term t pp_typ ty ; 
    begin match t with 
      ...
      | Equals(t1, t2) -> Format.eprintf "(Equals) @."; (*ignore(typecheck (whites + 2) gamma ct (Some ty) t1) ; ignore(typecheck (whites + 2) gamma ct (Some ty) t2) ; *)      
        ignore(typecheck (whites + 2) gamma ct (Some TBool) t1);  (*meter ignore*)
        ignore(typecheck (whites + 2) gamma ct (Some TBool) t2);   
...
let bank_constructor : construtor_ = ([param_balances],TmReturn(TmAssign("balances", param_balances_val) )) 

let deposit : func = { return_type = TUnit; name = "deposit"; params = []; body = TmReturn( TmMappAss(balances, sender, TmPlus( TmMappSel(balances, sender) , msg_value) ) ) }
...
let transfer : func = {return_type = TUnit; name = "transfer"; params = [param_to; param_amount]; body = TmReturn( TmIf( GreaterOrEquals(TmMappSel(balances, sender), param_amount_val), 
                                                                      TmSeq( TmMappAss(balances, sender, TmMinus( TmMappSel(balances, sender),param_amount_val)),
                                                                      TmMappAss(balances, param_to_val, TmPlus( TmMappSel(balances, param_to_val),param_amount_val))), 
...
let withdraw : func = {return_type = TUnit; name = "withdraw"; params = [param_withdraw_amount]; body = TmReturn( TmIf( GreaterOrEquals(TmMappSel(balances, sender), param_withdraw_amount_val), 
...
let marketplace_constructor : construtor_ = ([param_description;param_price], 
                                     TmReturn(TmSeq( TmAssign("InstanceOwner",param_description_val) , TmSeq(TmAssign("AskingPrice", param_price_val ), TmAssign("Description", param_description_val) )) ) )
...
let make_offer : func = {return_type = TUnit; name = "make_offer"; params = [param_offer]; 
                      body = TmReturn( TmSeq( TmIf(Equals(param_offer_val, TmValue(VCons(0))), TmRevert, TmReturn(TmValue(VUnit))),TmSeq( TmIf(Equals(sender, instanceOwner), TmRevert, TmReturn(TmValue(VUnit))),
                      TmSeq( TmAssign("InstanceBuyer", sender), TmAssign("OfferPrice", param_offer_val) ) )))}
                

let param_offer_r : params = (TNat, "param_offer_r")
let param_offer_r_val : term = TmGetParam("param_offer_r")                      
let reject : func = {return_type = TUnit; name = "reject"; params = [param_offer_r]; body = TmReturn(TmSeq( TmIf(Not(Equals(param_offer_r_val, TmValue(VCons(0)))), 
                                                              TmRevert, 
                                                              TmReturn(TmValue(VUnit))), 
...
let param_offer_a_val : term = TmGetParam("param_offer_a")    
